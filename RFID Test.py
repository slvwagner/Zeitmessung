# === Hardened RC522 UID reader (no OLED) — Pico2 W, MicroPython ===
...
def init_rfid():
    ...
    _wr(TxModeReg, 0x00)    # ensure CRC handling via FIFO only
    _wr(RxModeReg, 0x00)    # match ISO14443A defaults
    antenna_on()
...
def reqa():
    _wr(BitFramingReg, 0x07)     # 7-bit
    _wr(CollReg, 0x80)           # clear collisions
    return _to_card([PICC_CMD_REQA])

def anticoll_level(sel_code):
    _wr(BitFramingReg, 0x00)
    _wr(CollReg, 0x80)           # clear collisions before anticollision
...
def select_with_crc(sel_code, uid4, tries=6):
    ...
    for t in range(tries):
        ...
        if s == MI_OK and len(resp) >= 1:
            return MI_OK, resp[0]
        # If truly nothing came back, just back off and retry quietly
...
def get_uid_bytes():
    ...
    if s != MI_OK:
        if PRINT_DEBUG: print("DBG: CL1 anticoll failed")
        return None

    uid = bytearray()
    if part[0] == 0x88:  # cascade in CL1 -> 7 or 10 bytes
        uid += bytes(part[1:4])
        s, sak = select_with_crc(PICC_SEL_CL1, [0x88, uid[0], uid[1], uid[2]])
        if s != MI_OK:
            if PRINT_DEBUG: print("DBG: CL1 select failed")
            return None
        s, part2 = anticoll_retry(PICC_SEL_CL2)
        if s != MI_OK:
            if PRINT_DEBUG: print("DBG: CL2 anticoll failed")
            return None
        if part2[0] == 0x88:
            uid += bytes(part2[1:4])
            s, sak = select_with_crc(PICC_SEL_CL2, [0x88, part2[1], part2[2], part2[3]])
            if s != MI_OK:
                if PRINT_DEBUG: print("DBG: CL2 select (cascade) failed")
                return None
            s, part3 = anticoll_retry(PICC_SEL_CL3)
            if s != MI_OK:
                if PRINT_DEBUG: print("DBG: CL3 anticoll failed")
                return None
            uid += bytes(part3[0:4])
            s, sak = select_with_crc(PICC_SEL_CL3, [uid[-4], uid[-3], uid[-2], uid[-1]])
            if s != MI_OK:
                if PRINT_DEBUG: print("DBG: CL3 select failed")
                return None
        else:
            uid += bytes(part2[0:4])
            s, sak = select_with_crc(PICC_SEL_CL2, [uid[3], uid[4], uid[5], uid[6]])
            if s != MI_OK:
                if PRINT_DEBUG: print("DBG: CL2 select failed")
                return None
    else:
        uid += bytes(part[0:4])
        s, sak = select_with_crc(PICC_SEL_CL1, [uid[0], uid[1], uid[2], uid[3]])
        if s != MI_OK:
            if PRINT_DEBUG: print("DBG: CL1 select failed (4-byte)")
            return None
...
while True:
    ...
    if uid:
        ...
        while True:
            s2, _, bits2 = reqa()
            if not (s2 == MI_OK and bits2 == 0x10):
                break
            time.sleep_ms(80)
    time.sleep_ms(40)
