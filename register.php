...
// Load PHPMailer (using /src/ because that's where the files are)
...
if (isset($_GET['token'])) {
    ...
    $sql = "SELECT Startnummer, token_expires 
            FROM participant 
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($name) || empty($vorname) || !$email) {
        ...
        $sql = "INSERT INTO participant 
                (Name, Vorname, Nickname, Phone, `E-mail`, Kategorie, Gewicht, verification_token, token_expires) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
        $stmt = $conn->prepare($sql);
