<?php
// insert_race.php — inserts a row into `race` (with robust API key debug)
...
function to_mysql_datetime_ms($val) {
    ...
    if (preg_match('/^\d{12,}$/', (string)$val)) {
        ...
        return $dt->format('Y-m-d H:i:s.u'); // MySQL rounds to ms
    }
...
// ===== QUICK PING (confirm this exact file is running) =====
if (($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'GET' && isset($_GET['ping'])) {
    $hdrs = function_exists('getallheaders') ? getallheaders() : [];
    echo json_encode([
        'ok' => true,
        'msg' => 'insert_race.php alive',
        'version' => 'v3-debug',
        'has_getallheaders' => (bool)function_exists('getallheaders'),
        'headers_seen' => $hdrs,
        'server_vars_present' => [
            'HTTP_X_API_KEY' => isset($_SERVER['HTTP_X_API_KEY']),
...
// ===== AUTH (accept header OR JSON body OR form OR GET during debug) =====
$received = '';
$hdrs = function_exists('getallheaders') ? getallheaders() : [];
if (isset($hdrs['X-API-Key'])) {
    $received = (string)$hdrs['X-API-Key'];
}
if ($received === '' && isset($_SERVER['HTTP_X_API_KEY'])) {
...
// DEBUG: show exactly what we got if mismatch
