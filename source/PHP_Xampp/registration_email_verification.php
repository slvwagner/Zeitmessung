...
// Email settings (configure these according to your environment)
    ...
    if (empty($name) || empty($vorname) || !$email) {
        ...
        if ($stmt->execute()) {
            ...
            if (sendVerificationEmail($email, $name, $vorname, $verification_token, $smtp_host, $smtp_username, $smtp_password, $from_email, $from_name)) {
                ...
                $error_message = "Registration successful but we couldn't send the verification email. Please contact support.";
            }
        } else {
...
function sendVerificationEmail($to_email, $name, $vorname, $token, $smtp_host, $smtp_username, $smtp_password, $from_email, $from_name) {
    $verification_url = "http://" . $_SERVER['HTTP_HOST'] . $_SERVER['PHP_SELF'] . "?token=" . $token;
...
/*
function sendVerificationEmailWithPHPMailer($to_email, $name, $vorname, $token, $smtp_host, $smtp_username, $smtp_password, $from_email, $from_name) {
...
<body>
    <div class="container">
        ...
        <?php elseif (isset($verification_error)): ?>
            ...
            <a href="<?php echo $_SERVER['PHP_SELF']; ?>" class="back-link">Try again</a>
