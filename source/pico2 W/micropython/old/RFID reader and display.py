# === RC522 + OLED demo (Pico2 W, MicroPython) ===
# Uses your working RC522 UID reader + safe OLED writes
# OLED: your OLED.py (I2C0 on GP4/GP5 @ 50 kHz)
# RC522: SPI1 -> SCK=GP10, MOSI=GP11, MISO=GP12, CS(SDA)=GP13, RST=GP22

from machine import Pin, SPI
import time, OLED

# ---------- CONFIG ----------
HEARTBEAT_MS = 1000        # how often to show a tiny heartbeat when idle
HEARTBEAT_RF_QUIET = False # False = keep RF on for heartbeat writes
UID_HOLD_MS = 1500         # how long to keep UID on screen before going back to idle

# ---------- OLED FIRST ----------
OLED.oled_init()

def oled_show(lines, rf_quiet=True):
...
def _to_card(send, wait_loops=12000, settle_us=0):
    ...
    _wr(CommandReg, PCD_TRANSCEIVE); _set(BitFramingReg, 0x80)
...
def get_uid_bytes():
    # Quick presence check (don’t over-gate)
    s, _, bits = reqa()
...
print("Present a tag…")
oled_show(["RC522 + OLED", "Present a tag", ""], rf_quiet=True)
...
while True:
    ...
    if uid:
        ...
        if hx != last_uid:
            ...
            oled_show(["TAG DETECTED", hx, ""], rf_quiet=True)
            last_uid = hx

        # Wait until tag is removed (no extra OLED writes here)
        while True:
            s2, _, b2 = reqa()
        ...
        # After removal, return to idle screen
        oled_show(["RC522 + OLED", "Present a tag", ""], rf_quiet=True)
        continue

    # Gentle heartbeat when idle (avoid spamming & avoid RF mute)
    now = time.ticks_ms()
    if time.ticks_diff(now, last_hb) >= HEARTBEAT_MS:
        last_hb = now
        oled_show(["RC522 + OLED", "Present a tag…", ""], rf_quiet=HEARTBEAT_RF_QUIET)
