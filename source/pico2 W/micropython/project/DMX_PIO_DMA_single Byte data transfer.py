...
#
# Single-SM architecture: one PIO state machine generates BREAK, MAB, and
# serial data slots. A DMA channel streams bytes directly into that SM TX FIFO,
# paced by DREQ so FIFO feeding stays aligned with serializer consumption.
#
...
PIO_BLOCK           = 0
SM_TX               = 0
SM_TX_CLOCK_HZ      = 1_000_000

# ---------------------------------------------------------------------------
# DMA constants derived from SM_TX
#
...
_PIO0_TXF0      = _PIO0_BASE + 0x10 + SM_TX * 0x04     # 0x50200010
_DREQ_PIO0_TX0  = SM_TX                                 # 0
...
def sm_DMX_tx():
    """SM0: Wait for CPU IRQ0, generate Break/MAB, then serialise all slots."""
    BREAK = 23
    MAB   = 12
...
class DMXControllerPIO_DMA:
    """
    DMX512 transmitter using one PIO state machine and one DMA channel.

    The TX SM (SM0) generates Break/MAB and serialises slots at 250 kbps.
    A DMA channel streams one byte per DMX slot directly into the SM0 TX FIFO,
    paced automatically by DREQ_PIO0_TX0.
    ...
    def __init__(self, tx_pin=0, channels=512, refresh_rate=43):
        ...
        if DEBUG:
            ...
            print(f"DMA channel: {self.dma.channel}  DREQ: {_DREQ_PIO0_TX0}  "
                f"FIFO addr: 0x{_PIO0_TXF0:08X}")
    ...
    def start(self):
        ...
        # Load slot-count into TX SM FIFO, then kick SM past the
        # one-shot initialisation sequence (instructions 1-3).
        try:
            self.sm_tx.put(self.n_slots)
        except Exception as e:
            print(f"Error loading TX SM FIFO: {e}")
            self.transmitting = False
            return
    ...
    def update_frame(self, timer):
        """Timer callback: snapshot frame, arm DMA, trigger TX SM.

        The DMA streams tx_frame → PIO0 SM0 TX FIFO, paced by DREQ so it
        automatically refills the FIFO as the TX SM consumes slots.
        ...
        try:
            ...
            self.dma.write = _PIO0_TXF0         # destination: PIO0 SM0 TX FIFO
            self.dma.count = len(self.tx_frame) # number of byte transfers
            self.dma.active(1)                  # start — DREQ paces the flow
            # ————————————————————————————————————————————————————————————

            # Trigger TX SM to begin Break → MAB → data sequence
            self.force_pio_irq0()
    ...
    def status(self):
        ...
        print(f"DMA DREQ:      {_DREQ_PIO0_TX0}")
        print(f"DMA FIFO addr: 0x{_PIO0_TXF0:08X}")
        print("\nFirst 8 channels:")
        ...
        if self.transmitting:
            try:
                print(f"\nFIFO status:")
                print(f"  TX SM FIFO:         {self.sm_tx.tx_fifo()} / 8 entries")
            except Exception:
                pass
        print("=" * 40)
